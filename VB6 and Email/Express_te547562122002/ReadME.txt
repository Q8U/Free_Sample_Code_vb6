
Hi and Assalamu Alaikum

Express Test is a Computerized Testing Application
having features like..
 
Maintaining Question Database,
Maintaining student/instructor logins,
Assigning test to students,
Generating reports (html) of student/instructor performance.   

its having attractive oval shaped forms and a lot of interesting code    
important MDB files 
1.login.exp ( actually an MDB file )
2.mdb.exp
3.bank.mdb( a sample Qusetion Database )

Sample Instructor------------------------ 
User ID    : SYSTEM 	Password : 123456
Sample Student---------------------------
User ID    : ssi	Password : 123456
-----------------------------------------
For more details please refer User_manual.htm file 

					Rafay
					rafaymansoor@yahoo.com
