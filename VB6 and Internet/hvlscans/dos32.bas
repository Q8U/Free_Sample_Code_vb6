...
MsgBox "You have " & Count & " mails", vbInformation, "Count Mail"
End Function
...
thelist.AddItem AddMails& + 1 & "]•   " & MyString$
    Next AddMails&
MsgBox "Done, you can now minimize the mail box", 16, "Done"
End Sub
...
Public Sub MailToListNew(thelist As ListBox)
    ...
    For AddMails& = 0 To Count& - 1
        ...
        thelist.AddItem AddMails& + 1 & "]•   " & MyString$
    Next AddMails&
MsgBox "Donem you can now minimize the mail box", 16, "Done"
End Sub
...
Public Function CheckAlive(ScreenName As String) As Boolean
    ...
    If InStr(LCase(ReplaceString(ErrorString$, " ", "")), LCase(ReplaceString(ScreenName$, " ", ""))) > 0 Then
        CheckAlive = False
ChatSend "</b>‹¬­› [" & ScreenName$ & "] ­› is dead"
    Else
        CheckAlive = True
ChatSend "</b>‹¬­› [" & ScreenName$ & "] ­› is alive!"
    End If
...
Public Function RoomCount2() As Long
    Dim AOL As Long, MDI As Long, rMail As Long, rList As Long
    Dim Count As Long
    AOL& = FindWindow("AOL Frame25", vbNullString)
    MDI& = FindWindowEx(AOL&, 0&, "MDICLIENT", vbNullString)
    ...
    Count& = SendMessage(rList&, LB_GETCOUNT, 0&, 0&)
    RoomCount2& = Count&
ChatSend "</b>‹¬­› åo-drågulà ­› by: hydra"
Pause 0.3
ChatSend "</b>‹¬­› [" & Count& & "] peeps ­› in this room"
End Function
...
Public Sub sup_bot(li As ListBox)
Call AddRoomToListbox(li, False)
Dim X%
For X% = 0 To li.ListCount - 1
ChatSend ("‹¬­› sup ­› " & li.List(X%) & "")
Pause (0.75)
Next X%
End Sub


Public Sub mass_im(lis As ListBox)
Call AddRoomToListbox(lis, False)
Dim X%
For X% = 0 To lis.ListCount - 1
ChatSend ("sup " & lis.List(X%) & "")
Pause (0.75)
Next X%
End Sub
...
'Public Sub ChatunIgnoreByName(name As String)
'    On Error Resume Next
'    Dim cProcess As Long, itmHold As Long, ScreenName As String
'    Dim psnHold As Long, rBytes As Long, index As Long, Room As Long
'    Dim rList As Long, sThread As Long, mThread As Long
...
'    mThread& = OpenProcess(PROCESS_READ Or RIGHTS_REQUIRED, False, cProcess&)
'    If mThread& Then
'        For index& = 1 To SendMessage(rList, LB_GETCOUNT, vbNull, 1, 1) - 1
'            ScreenName$ = String$(4, vbNullChar)
'            itmHold& = SendMessage(rList, LB_GETITEMDATA, ByVal CLng(index&), ByVal 0&)
'            itmHold& = itmHold& + 24
'            Call ReadProcessMemory(mThread&, itmHold&, ScreenName$, 4, rBytes)
...
Public Function CheckIMs(Person As String) As Boolean
    ...
    If InStr(oString$, "is online and able to receive") <> 0 Then
        CheckIMs = True
ChatSend "</b>‹¬­› [" & Person$ & "] ­› ims are on"
    Else
        CheckIMs = False
ChatSend "</b>‹¬­› [" & Person$ & "] ­› ims are off"
    End If
...
Public Function CheckIfMaster() As Boolean
    ...
    If mString$ = "Set Parental Controls" Then
        CheckIfMaster = True
ChatSend "</b>‹¬­› [" & GetUser & "] ­› is a master"
    Else
        CheckIfMaster = False
ChatSend "</b>‹¬­› [" & GetUser & "] ­› is not a master"
...
Public Function ProfileGet(ScreenName As String) As String
   ...
   MsgBox "They dont have a profile =(", 16, "Profile Check"
    Else
        Call PostMessage(pWindow&, WM_CLOSE, 0&, 0&)
        Call PostMessage(pgWindow&, WM_CLOSE, 0&, 0&)
        ProfileGet$ = pString$
